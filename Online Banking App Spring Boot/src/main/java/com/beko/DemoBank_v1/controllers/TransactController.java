...
public class TransactController {
    ...
    public ResponseEntity deposit(@RequestBody Map<String, String> requestMap, HttpSession session) {
        ...
        //TODO: UPDATE BALANCE:
Double fetchedBalance = accountRepository.getAccountBalance(user_id, acc_id);
        
        if (fetchedBalance == null) {
            return ResponseEntity.badRequest().body("Account not found. Please check your Account ID.");
        }
    ...
    ResponseEntity transfer(@RequestBody TransferRequest request, HttpSession session) {
        ...
        if (transfer_from.isEmpty() || transfer_to.isEmpty() || transfer_amount.isEmpty()) {
            return ResponseEntity.badRequest().body("The accounts and amount cannot be empty!");
        }
        ...
        // 5. SAFE CHECK FOR SOURCE ACCOUNT
        Double fetchedBalanceFrom = accountRepository.getAccountBalance(user_id, transferFromId);
        if (fetchedBalanceFrom == null) {
            return ResponseEntity.badRequest().body("Source account not found. Please check your Account ID.");
        }
        double currentBalanceOfAccountTransferringFrom = fetchedBalanceFrom;

        // 6. CHECK INSUFFICIENT FUNDS
        ...
        // 7. SAFE CHECK FOR TARGET ACCOUNT
        Double fetchedBalanceTo = accountRepository.getAccountBalance(user_id, transferToId);
        if (fetchedBalanceTo == null) {
            return ResponseEntity.badRequest().body("Target account not found or does not belong to you.");
        }
        double currentBalanceOfAccountTransferringTo = fetchedBalanceTo;
        ...
        // 10. LOG SUCCESSFUL TRANSACTION
        transactRepository.logTransaction(transferFromId, "Transfer", transferAmount, "online", "success", "Transfer Transaction Successful", LocalDateTime.now());
